...
// --- CONFIGURATION: IMMUTABLE SECURITY CREDENTIALS ---
...
// --- 1. LOGIN COMPONENT (Biometric & Forensic Security) ---
const Login = ({ setAuth }) => {
  ...
  const handleLogin = async (e) => {
    ...
    setTimeout(() => {
      if (username === 'user' && password === 'user') {
        ...
        notify("🚨 BREACH_ATTEMPT", `Access Denied for: ${username}/${password}`);
  ...
  return (
    <div className="login-gate">
      <style>{`
        .login-gate { min-height: 100vh; display: flex; align-items: center; justify-content: center; background: #020617; position: relative; overflow: hidden; font-family: 'Inter', sans-serif; }
        ...
        .cyber-input { background: #0f172a; border: 1px solid #1e293b; color: #fff; border-radius: 12px; padding: 14px; margin-bottom: 20px; width: 100%; outline: none; transition: 0.3s; font-family: monospace; }
      ...
      <div className="auth-card">
        ...
        <h2 className="text-white fw-bold mb-5">AXON_OS <span className="text-primary">v5.5</span></h2>
...
// --- 2. DASHBOARD COMPONENT (Intelligence & Productivity) ---
const Dashboard = ({ setAuth }) => {
  ...
  useEffect(() => {
    ...
    const fetchNews = async () => {
      ...
      } catch (e) { console.error("News Feed Offline"); }
    };
    fetchData(); fetchNews();
  ...
  return (
    <div className="dash-container">
      <style>{`
        ...
        .glass-panel { background: rgba(30, 41, 59, 0.4); backdrop-filter: blur(12px); border: 1px solid rgba(255,255,255,0.05); border-radius: 20px; padding: 25px; height: 100%; }
        ...
        .news-pill { border-left: 3px solid #ef4444; background: rgba(239, 68, 68, 0.05); padding: 10px; margin-bottom: 10px; border-radius: 0 8px 8px 0; font-size: 0.8rem; cursor: pointer; }
        .efficiency-pill { background: #3b82f6; color: white; font-weight: 800; padding: 5px 15px; border-radius: 20px; font-size: 0.8rem; }
      `}</style>
      ...
      <nav className="top-nav">
        ...
        <div className="d-flex gap-3">
          ...
          <button onClick={() => { setAuth(false); localStorage.removeItem('axon_auth'); navigate('/login'); }} className="btn btn-sm btn-outline-danger px-3">DISCONNECT</button>
        </div>
      </nav>
      ...
      <div className="container py-5 px-lg-5">
        <div className="row g-4">
          <div className="col-lg-7">
            <div className="glass-panel">
              <div className="d-flex justify-content-between align-items-center mb-4">
                <h5 className="fw-bold m-0"><i className="bi bi-cpu text-primary me-2"></i>Command Directives</h5>
                <span className="efficiency-pill">{completion}% Completion</span>
              </div>
              <div className="input-group mb-4">
                <input type="text" className="form-control bg-dark text-white border-0" placeholder="Initialize directive..." value={input} onChange={e => setInput(e.target.value)} onKeyPress={e => e.key === 'Enter' && addTask()} />
          ...
          <div className="col-lg-5">
            <div className="glass-panel">
              <h6 className="fw-bold text-danger mb-3"><i className="bi bi-broadcast me-2"></i>Iran Intelligence Feed</h6>
              {news.map((n, i) => (
...
// --- 3. READING HUB COMPONENT (Cognitive Recovery) ---
const ReadingHub = () => {
  const [bookmarks, setBookmarks] = useState([]);
  const [newB, setNewB] = useState({ title: '', page: '', note: '' });
  const navigate = useNavigate();
  const quotes = ["Failure is data, not destiny.", "Rebuild your code.", "Read to rebuild.", "One page at a time."];
  ...
  return (
    <div className="hub-wrap">
      <style>{`
        ...
        .float-text { position: absolute; font-size: 5rem; font-weight: 900; color: rgba(59, 130, 246, 0.03); white-space: nowrap; animation: float 60s linear infinite; pointer-events: none; }
        ...
        .input-dark { background: #0f172a; border: 1px solid #1e293b; color: white; border-radius: 12px; padding: 12px; margin-bottom: 12px; width: 100%; outline: none; }
        .btn-hub { background: #3b82f6; color: white; border: none; padding: 12px; border-radius: 12px; font-weight: bold; width: 100%; }
        .recovery-panel { border-left: 4px solid #f59e0b; background: rgba(245, 158, 11, 0.05); padding: 20px; border-radius: 0 16px 16px 0; margin-bottom: 30px; }
      `}</style>
      
      {quotes.map((q, i) => <div key={i} className="float-text" style={{top: `${i * 25}%`, animationDelay: `${i * 10}s`}}>{q}</div>)}
      ...
      <div className="container" style={{maxWidth: '1100px'}}>
        <div className="d-flex justify-content-between align-items-center mb-5" style={{position: 'relative', z-index: 20}}>
          ...
          <button className="btn btn-outline-light px-4" onClick={() => navigate('/dashboard')}>EXIT_TO_CORE</button>
        </div>

        <div className="recovery-panel">
          <h4 className="text-warning fw-bold mb-3"><i className="bi bi-lightning-charge-fill me-2"></i>Neural Reconstruction</h4>
          <p className="text-secondary m-0">Failure is just a system crash. Reading is how you re-install the OS. Every page is a patch. Every book is a kernel update. Rebuild your source code, Shifanth.</p>
        </div>

        <div className="row g-4">
          <div className="col-lg-4">
            <div className="glass-box">
              <h5 className="fw-bold mb-4 text-white">Log Neural Bookmark</h5>
              ...
              <input type="number" className="input-dark" placeholder="Page Number" value={newB.page} onChange={e => setNewB({...newB, page: e.target.value})} />
              <textarea className="input-dark" placeholder="Eureka Insight..." style={{height: '100px'}} value={newB.note} onChange={e => setNewB({...newB, note: e.target.value})} />
              <button className="btn-hub" onClick={saveB}>SAVE_BOOKMARK</button>
            </div>
          </div>
          <div className="col-lg-8">
            <div className="glass-box">
              <h5 className="fw-bold mb-4 text-white">Knowledge Library</h5>
              <div className="row g-3">
                {bookmarks.map((b, i) => (
                  <div key={i} className="col-md-6">
                    <div className="p-3 rounded bg-dark border border-secondary">
                      <div className="d-flex justify-content-between small text-primary mb-2"><span>PG. {b.page}</span><i className="bi bi-bookmark-fill"></i></div>
                      <div className="fw-bold text-white mb-1">{b.title}</div>
