...
// --- 1. LOGIN COMPONENT (With IP tracking & Apple-style UI) ---
const Login = ({ setAuth }) => {
  ...
  const handleLogin = async (e) => {
    ...
    // 1. Fetch IP and Location Data (ipapi.co)
  ...
  return (
    <div className="min-vh-100 d-flex align-items-center justify-content-center" style={{ backgroundColor: '#0f172a' }}>
      <style>{`
        ...
        .apple-input { background: rgba(255, 255, 255, 0.07); border: 1px solid rgba(255, 255, 255, 0.1); color: white; border-radius: 14px; padding: 14px 20px; margin-bottom: 15px; transition: 0.2s; }
        .apple-input:focus { background: rgba(255, 255, 255, 0.12); border-color: #6366f1; color: white; box-shadow: none; }
        .apple-btn { background: #6366f1; border: none; border-radius: 14px; padding: 14px; font-weight: 600; width: 100%; margin-top: 10px; transition: 0.2s; }
      ...
      <div className={`apple-card ${isShaking ? 'shake' : ''}`}>
        <div className="mb-4"><i className="bi bi-cpu text-primary" style={{ fontSize: '3.5rem' }}></i></div>
        <h2 className="fw-bold text-white mb-1">AXON <span className="text-primary">OS</span></h2>
        <p className="text-secondary small mb-5">Kernel Access Protocol v3.4</p>
        ...
        <div className="creator-tag">
          ...
          <div className="small text-primary mt-1" style={{ fontSize: '0.65rem' }}><i className="bi bi-patch-check-fill me-1"></i>Software Engineer (Remote)</div>
        </div>
      </div>
    </div>
  );
    ...
    <div className="min-vh-100 d-flex flex-column" style={{ backgroundColor: '#0f172a', color: '#f8fafc', fontFamily: "'Inter', sans-serif" }}>
      ...
      <nav className="px-4 py-3 d-flex justify-content-between align-items-center border-bottom border-secondary border-opacity-25">
        ...
        <div className="d-flex align-items-center gap-3">
          <div className="small text-secondary d-none d-md-block">{liveTime.toLocaleTimeString()}</div>
          <button onClick={handleLogout} className="btn btn-sm btn-outline-danger" style={{ borderRadius: '10px' }}><i className="bi bi-power"></i></button>
        </div>
      </nav>
      ...
      <div className="container py-5" style={{ maxWidth: '700px' }}>
        ...
        <div className="glass-card p-4 mb-4 shadow-lg">
          <div className="input-group mb-3">
            ...
            <button className="btn btn-primary" onClick={addTask} style={{ background: '#6366f1', border: 'none' }}><i className="bi bi-cloud-arrow-up"></i></button>
          </div>
          <div className="d-flex gap-2">
            {categories.map(cat => (<button key={cat} onClick={() => setCategory(cat)} className={`btn btn-sm ${category === cat ? 'btn-light' : 'btn-outline-secondary text-white'}`} style={{ borderRadius: '20px', fontSize: '0.75rem' }}>{cat}</button>))}
          </div>
        </div>
        ...
        <div className="glass-card overflow-hidden shadow-lg">
          <div className="p-3 border-bottom border-secondary border-opacity-10 d-flex justify-content-between">
            ...
            <span className="small text-secondary">{tasks.length} Nodes</span>
          </div>
            ...
            {tasks.map((task) => (
              <div key={task.id} className="task-item d-flex align-items-center p-3 border-bottom border-secondary border-opacity-10">
                ...
                <div className="flex-grow-1" onClick={() => toggleTask(task.id, task.completed)} style={{cursor: 'pointer'}}>
                  <div className={task.completed ? 'task-completed' : ''}><span className="category-pill me-2">{task.category}</span> {task.text}</div>
                </div>
...
// --- 3. MAIN ROUTER (The Persistent Gate) ---
